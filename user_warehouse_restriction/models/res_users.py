...
class ResUsers(models.Model):
    ...
    location_ids = fields.Many2many(
        ...
        domain="[('location_warehouse_id', 'in', allowed_warehouse_ids)]")

    @api.onchange('allowed_warehouse_ids', 'restrict_location')
    def _onchange_allowed_warehouse_ids(self):
        if not self.restrict_location:
            self.location_ids = False
        elif self.restrict_location and not self.location_ids:
            self.location_ids = self.env['stock.location'].search([
                ('location_warehouse_id', 'in', self.allowed_warehouse_ids.ids)])
    ...
    def _compute_check_user(self):
        ...
        for record in self:
            record.check_user = restriction_group_id in record.groups_id.ids
